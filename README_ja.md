...
flowchart TD
    ...
    B -->|"最大 N 件の BATCH PLAN"| D["2 · IMPROVE<br/>項目ごと: バックアップ → 修正 → チェックポイント検証 → 保持/巻き戻し"]
...
1. **ANALYZE** *(厳格に読み取り専用)* — 実ソースを残らず読み、`file:line` で根拠を示した具体的な指摘を順位づけし、
   **独立して巻き戻せる**項目を最大 `improvementBatchSize` 件まで収めた **BATCH PLAN** を作ります。そしてドライバへ
   ...
   それは次のフェーズの仕事です。
2. **IMPROVE** *(ANALYZE が何か見つけたときだけ)* — 計画を**項目ごと**に処理します。触れるファイルをまず
   バックアップし、最小で正しい変更を加え、チェックポイント単位で検証します。リグレッション（またはテスト/リント
