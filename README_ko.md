...
flowchart TD
    ...
    B -->|"최대 N개 BATCH PLAN"| D["2 · IMPROVE<br/>항목별: 새 구현 세션 → 새 검토 세션 → 보존/되돌림"]
...
1. **ANALYZE** *(엄격히 읽기 전용)* — 실제 소스를 빠짐없이 읽고, `file:line`으로 근거를 단 구체적 발견들을 순위 매긴
   ...
   단계의 몫입니다.
2. **IMPROVE** *(ANALYZE가 뭔가 찾았을 때만)* — 계획을 **항목별로** 처리합니다. 드라이버가 항목 하나의 새 구현 세션을
   실행한 뒤, 구현 의견을 신뢰하지 않고 검증·벤치마크를 다시 수행하는 별도의 새 검토 세션을 실행합니다. 검토 판정만
   최종 결과가 되며 그 뒤에 다음 항목을 시작합니다. 회귀가 나면 **그 항목만** 백업에서
...
| `analysisAgent` / `improveAgent` / `reviewAgent` | 분석·구현·검토 역할별 CLI; 생략 시 `agent`, 검토는 먼저 `improveAgent`로 폴백 | `agent` |
| `model` | `provider/model` id; 비우면 에이전트 기본값 | `""` |
| `analysisModel` / `improveModel` / `reviewModel` | 역할별 모델; 같은 CLI일 때만 이전 역할/공용 model을 상속하고, 다른 CLI면 해당 agent 기본값 사용 | `""` |
| `analysisVariant` / `improveVariant` / `reviewVariant` | 역할별 추론 강도; 같은 CLI일 때만 이전 역할/공용 variant를 상속하고, 다른 CLI면 자체 최고 등급 자동 | `""` |
...
| `items/NNNN-<item>-<stage>-*` | 항목별 구현·검토 세션의 격리된 current/signal/prompt 산출물 |
...
| `HEAD_CHANGED.json` | 에이전트의 무단 커밋을 지속 격리; 기대 `HEAD` 복원 시 자동 해제되고 `retry-now reset`으로 명시 해제 가능 |
...
- **회귀는 자동으로 롤백됩니다** — 체크포인트(테스트/린트) 실패나 벤치마크 회귀가 나면 문제의 항목만 되돌리고, 빌드는
  항상 초록으로 남깁니다.
- **트랜잭션 범위는 Git-visible 파일입니다** — 추적 파일과 일반 미추적 파일은 모드·심볼릭 링크·원시 Git 인덱스까지
  바이트 단위로 복원합니다. `.env`, 캐시, 생성물 같은 ignore 파일은 이 보장 밖이므로 별도로 보호해야 합니다.
- **서브모듈/gitlink는 에이전트 실행 전에 거부합니다** — mode `160000` 항목이 있으면 안전한 스냅샷을 만들 수 없으므로
  ANALYZE를 시작하지 않습니다.
- **예상치 못한 커밋은 자동 reset하지 않고 격리합니다** — 에이전트가 `HEAD`를 바꾸면 즉시 중단하고, 기대한 리비전을
  복원하거나 `retry-now reset`으로 현재 상태를 명시적으로 수락하기 전까지 재실행을 차단합니다.
- **중단된 배치는 윤회 전체를 롤백합니다** — 뒤 항목이 크래시하거나 쿼터가 소진되면 완료되지 않은 배치의 앞선 검토 통과
  항목도 시작 상태로 복원하여, 기록되지 않은 부분 진행을 다음 실행이 몰래 받아들이지 않게 합니다.
